...
const BUG_STATUSES = ['New', 'Triaged', 'In Progress', 'Fixed', 'Verified', 'Closed', 'Reopened'] as const
const SEVERITIES = ['Low', 'Medium', 'High', 'Critical'] as const
const PRIORITIES = ['Low', 'Medium', 'High', 'Critical'] as const

export function BugFilters(): JSX.Element {
  ...
  function toggleSeverity(severity: string): void {
    ...
    const next = current.includes(severity)
      ...
      : [...current, severity]
    setFilters({ ...filters, severity: next.length > 0 ? next : undefined })
  }
